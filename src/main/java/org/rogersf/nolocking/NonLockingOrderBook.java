...
public class NonLockingOrderBook {

	private final NonLockingPriceLevels bids;
	private final NonLockingPriceLevels asks;
	...
	private void matchOrders ( Order.Side aggressorSide ) {
		...
		NonLockingPriceLevels orders = aggressorSide == Order.Side.BUY ? bids : asks;
		if ( ! orders.empty () ) {
	...
	public NonLockingOrderBook ( final List < OrderBookListener > listener , final String ticker ) {
		this.listener = listener;
		bids = new NonLockingPriceLevels ( false );
		asks = new NonLockingPriceLevels ( true );
		this.ticker = ticker;
	}
	...
	private void snap ( final NonLockingPriceLevels src , List < BookLevel > dest , List < Long > orderIds ) {
